...
def entropy(y):
    ...
    probabilities = np.sum(y, axis=0) / y.shape[0]
...
def mad_median(y):
    ...
    return np.mean(np.sum(np.abs(y - np.median(y))))
...
class DecisionTree(BaseEstimator):
    ...
    def __init__(self, n_classes=None, max_depth=np.inf, min_samples_split=10,
        ...
        for i in range(X_subset.shape[1]):
            ...
            for thr in thresholds:
                ...
                functional = self.criterion(X_subset) - \
                             y_left.shape[0] / X_subset.shape[0] * self.criterion(y_left) - \
                             y_right.shape[0] / X_subset.shape[0] * self.criterion(y_right)
                if functional > best_func_value:
                    best_func_value = functional
                    feature_index = i
                    threshold = thr
