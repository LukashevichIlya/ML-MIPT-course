...
def rbf(x_1, x_2, sigma=1.):
    ...
    x_1 = x_1.unsqueeze(1).expand(n, m, d)
    x_2 = x_2.unsqueeze(0).expand(n, m, d)

    distances = torch.exp(-torch.pow(x_1 - x_2, 2).sum(2) / (2 * sigma ** 2))
...
def hinge_loss(scores, labels):
    ...
    return torch.mean(
        torch.max(torch.zeros(list(scores.size())[0]), torch.ones(list(scores.size())[0]) - torch.mul(scores, labels)))
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def linear(x_1, x_2):
        ...
        """
        return x_1.matmul(x_2.transpose(0, 1)).type(torch.float32)
